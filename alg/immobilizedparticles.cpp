...
void Immobilizedparticles::activate()   {
    ...
    //     switch (pState) {
    //     case State::processactivateLeaderElection:
    //         activateLeader();
    //         break;
    //     case State::processactivateInitializeTree:
    //         activateImmo();
    //         activateInitialTree();
    //         break;
    //     case State::processactivateMovetoLead:
    //         activateMove();
    //         break;
    //     case State::processactivateTarget:
    //         activateTarget();
    //         break;
    //     case State::processperformMovement:
    //         performMovement();
    //         break;
    //     case State::processactivateHex:
    //         activateHex();
    //         break;
    //     default:
...
//                         if (hasNbrAtLabel(label)) {
//                             auto& nbr = nbrAtLabel(label);
//                             if (nbr.isInState({State::Immo}) || nbr.isInState({State::Idle}) || nbr.isInState({State::Marker})) {
//                                 state = State::Marker;
//                                 printf("No leader or follower neighbor found, changing state to Marker.\n");
//                                 break;
//                             }
//                         }
//                     }
...
//                     if (hasNbrAtLabel(label)) {
//                         auto& nbr = nbrAtLabel(label);
//                         if (nbr.isInState({State::Leader}) || nbr.isInState({State::Follower})){
//                             //followDir = labelToDir(label);
//                             return;
//                         }
//                         else if (nbr.isInState({State::Immo})) {
...
//                             //nbr.moveDir = followDir;
//                         }
//                         if (nbr.isInState({State::Follower, State::Leader, State::Idle, State::Marker})) {
...
//                 printf("Particle is Marker. Forwarding communication without changing state.\n");
//                 for (int label : uniqueLabels()) {
//                     if (hasNbrAtLabel(label)) {
//                         auto& nbr = nbrAtLabel(label);
...
//                             return;
//                         }
//                         else if (nbr.isInState({State::Marker, State::Immo})) {
...
void Immobilizedparticles::initializeTrees(){
    ...
    // Leader realizes that tree initialization has finished
...
// #endif
    ...
    //         if (hp->state == Immobilizedparticles::State::Leader && hp->tree2Ack && hp->phase == Immobilizedparticles::Phase::MoveToTargetTree) {
    //             return true;
    //         }
    //     }
    // }

    // // Return false if no terminating condition is met
    // return false;
//}
        ...
        if (immobileParticle && immobileParticle->phase==Immobilizedparticles::Phase::LeaderElection) {
            for (int label : immobileParticle->uniqueLabels()) {
                if (immobileParticle->hasNbrAtLabel(label)) {
                    ...
                    if (nbr.isInState({Immobilizedparticles::State::Idle})) {
                        immobileParticle->isIdle = true;
                    }

                    if (nbr.isInState({Immobilizedparticles::State::Immo})) {
...
//     switch (state) {
//     case State::Leader:       return 0x39FF14; // Fluorescent Green
//     case State::Follower:     return 0x0000FF; // Blue
//     case State::Immo:        return 0x000000;  // black
//     case State::Marker:      return 0xFF8800; // Fluorescent Orange
//     case State::FollowerHex:  return 0x00FFEF; // Fluorescent Blue
//     case State::Seed:         return 0xFF1493; // Fluorescent Pink
//     case State::Root:         return 0xFF0000; // Red
//     case State::Retired:      return 0x808080; // Grey
//     case State::Single:       return 0xa100ff; // purple
//     case State::Finish:     return 0x800000; // maroon
//     case State::Lead:        return 0xa100ff; // purple
...
//         }
//     } else if (isInState({State::Immo})){
//         //_borderPointColors.fill(0x00cc00); // green
//         _borderPointColors.at(localToGlobalDir(tokenForwardDir)) = 0x00cc00; // green
//     }else {
//         _borderPointColors.fill(-1);
//     }
...
//     if (!immobilizedSystem) {
//         printf("Error: Particle system not found!\n");
//         return;
//     }

//     // Ensure proper phase transitions
//     if (!immobilizedSystem->hasLeader() && phase != Phase::LeaderElection) {
//         phase = Phase::LeaderElection;
//     } else if (immobilizedSystem->hasLeader() && phase == Phase::LeaderElection) {
...
//             if (nbr.state == State::Leader) {
//                 // Set both followDir1 and followDir2 if a Leader is found
//                 followDir1 = label;
//                 if (!followDir2Set) {
//                     followDir2 = label;
...
//                 }
//             } else if (nbr.followDir2 != -1) {
//                 // Update followDir2 only if it hasn't been set yet
//                 if (!followDir2Set) {
//                     followDir2 = label;
//                     followDir2Set = true; // Mark followDir2 as set
//                 }
...
// //     // Iterate through particles to check for termination condition
// //     for (const auto& p : particles) {
// //         auto hp = dynamic_cast<const Immobilizedparticles*>(p);
// //         if (hp) {
// //             // Check if all particles have gone through the initializeTrees phase
...
// //             }
// //             // Check for termination condition
// //             if (hp->state == Immobilizedparticles::State::Leader && hp->tree2Ack && hp->followDir2 !=-1 && hp->phase == Immobilizedparticles::Phase::MoveToTargetTree) {
// //                 return true;
// //             }
// //         }
// //     }
...
//     // The color of the circle around the particle.
//     switch (state) {
//     case State::Leader:       return 0x00FFEF; // Fluorescent Blue
//     case State::Follower:     return 0x0000ff; // blue
